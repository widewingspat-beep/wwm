...
export default function BlogsClient({ posts }: { posts: Post[] }) {
  ...
  /* ── Recent posts (always top 5 from all posts) ── */
  ...
  return (
    <section className="blg-section">
      <div className="container">
        ...
        {/* ── Category tabs ── */}
        <div className="blg-tabs" role="tablist" aria-label="Filter by category">
          {categories.map(cat => (
            <button
              key={cat}
              role="tab"
              aria-selected={activeCategory === cat}
              className={`blg-tab${activeCategory === cat ? ' blg-tab--active' : ''}`}
              onClick={() => changeCategory(cat)}
            >
              {cat}
              <span className="blg-tab-count">{catCounts[cat] ?? 0}</span>
            </button>
          ))}
        </div>
        ...
        <div className="blg-layout">
          ...
          <aside className="blg-sidebar">
            ...
            <div className="blg-widget">
              <h3 className="blg-widget-title">
                <span className="blg-widget-accent" />
                Recent Posts
              </h3>
            ...
            <div className="blg-widget">
              ...
              <ul className="blg-cat-list">
                {categories.map(cat => (
                  <li key={cat}>
                    <button
                      className={`blg-cat-item${activeCategory === cat ? ' blg-cat-item--active' : ''}`}
                      onClick={() => changeCategory(cat)}
                    >
                      <span className="blg-cat-name">{cat}</span>
